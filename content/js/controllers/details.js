...
function ($q, $rootScope, $scope, $log, AppSupport, DataFactory, displayHelper, $stateParams, $state, $transitions) {
    ...
    getTestData(testCatalogCd).then(function (response) {
        ...
        testSections = response[0].data//JSON.parse(response[0].data);
        fields = JSON.parse(response[1].data);//response[1].data
        hValues = response[2].data//JSON.parse(response[2].data);//
